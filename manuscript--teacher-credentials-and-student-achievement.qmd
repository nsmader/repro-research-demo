...
ggplot(ma_ed,
  ...
  labs(title = "District Rates of MCAS Proficiency by Teacher Experience",
       ...
       y = "% Meets/Exceeds",
       caption = str_wrap('Note: teachers are "experienced" if they have taught in a Massachusetts public school for at least three years.')) +
...
vX <- c("exp_pct_10pp", "avg_pay_10k", "stud_teach_ratio", "pov_pct_10pp")
...
fmt_reg <- function(fit) {
  tbl_regression(fit,
                 ...
                 conf.int = FALSE,
                 label = list(exp_pct_10pp     ~ "% Experienced Teachers (10 pct pts)",
                              avg_pay_10k      ~ "Average Teacher Pay ($1000s)",
                              stud_teach_ratio ~ "Student-to-Teacher Ratio",
...
When accounting for other regressors, a 10 percentage point increase in experienced teachers is associated with the percentage of students meets/exceeding MCAS proficiency standards is 
`r percent(lRegs[["ELA"]]$ coefficients["exp_pct_10pp"], accuracy = 0.1)`, 
`r percent(lRegs[["MATH"]]$coefficients["exp_pct_10pp"], accuracy = 0.1)`, and 
`r percent(lRegs[["SCI"]]$ coefficients["exp_pct_10pp"], accuracy = 0.1)` for English and Language Arts, Math, and Science respectively.
